...
export class SolicitudesComponent implements AfterViewInit {
    ...
    this.http.get<any>(url, { headers }).subscribe({
      next: (resp) => {
        ...
        if (!resp.success || resp.error || (resp.message && resp.message.toLowerCase().includes('error'))) {
          alert('La API devolvió un error: ' + (resp.message || resp.error || 'DNI no encontrado'));
        ...
        if (!result) {
          console.warn('[consultarReniecPorDni] No hay objeto result en la respuesta');
        ...
        if (nombreCompleto) {
          ...
          console.warn('[consultarReniecPorDni] No se encontraron campos de nombre en result');
          alert('No se encontraron datos para ese DNI. Verifica que el DNI sea correcto.');
          this.nombres_apellidos = '';
        }
      },
  ...
  consultarSunatPorRuc() {
    ...
    this.http.get<any>(url, { headers }).subscribe({
      next: (resp) => {
        console.log('[consultarSunatPorRuc] Respuesta completa de la API:', resp);
        ...
        const result = resp.result;
        if (!result) {
          console.warn('[consultarSunatPorRuc] No hay objeto result en la respuesta');
        ...
        console.log('[consultarSunatPorRuc] Datos extraídos:', { razonSocial, direccion });
